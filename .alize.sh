...
# alias m='cd /www/wwwroot/sxcg_backend'
alias gmr='git merge'
...
alias gclm='clo2(){ git clone git@gitee.com:AlmondLee/$@ && cd $@; };clo2 $@'
...
alias goi='go mod init ${PWD##*/} && wget https://gitee.com/AlmondLee/simple_mirror/raw/master/.gitignore && git init && git add -A &&git commit -m init && git checkout -b dev1 && git status'
# alias goi='go mod init this-project && wget https://gitee.com/AlmondLee/simple_mirror/raw/master/.gitignore && git init && git add -A &&git commit -m init && git checkout -b dev1 && git status'
...
export PATH=$PATH:$MY_GOROOT:$MY_JAVA_BIN:$MY_CLANGROOT:$MY_JRE_BIN:$MY_GO_BIN:$MY_MYSQL_BIN:$MY_CARGO_BIN
